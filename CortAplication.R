...
project_on_dims = function(object,dims) {
...
plot.Cort <- function(x,...){
  ...
  graphics::par(mfrow=c(1,1),
                mai = c(0,0,0,0),
                oma = c(3,3,5,3))
  graphics::plot(c(0,1),c(0,1),type="n",axes = TRUE,xlab="2",ylab="1",xaxt='n',yaxt='n')
  xx = dd[(dd$dim_x == 1)&(dd$dim_y==2),]
  graphics::rect(xx$ymin,xx$xmin,xx$ymax,xx$xmax,col=grDevices::gray(1-xx$col),border=NA,density=NA)
  graphics::points(x@data[,1],x@data[,2],cex=0.5,col="red")
}



#listaCorta<-AplikacjaCort(dane[,c(1,2)],min_node_size=2,verbose_lvl=1)
#length(listaCorta)
#plot.Cort(listaCorta[[4]])


#yy<-rCopula(5000,listaCorta[[5]])
#plot(yy)

#pair(cbind(yy,dane[,c(1,2)]))
